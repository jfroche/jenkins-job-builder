...
def _default_view(parser, xml_parent):
    def_view = XML.SubElement(xml_parent, 'hudson.model.AllView')

    " default not configurable options"
    owner = XML.SubElement(def_view, 'owner')
    owner.set('class', 'com.cloudbees.hudson.plugins.folder.Folder')
    owner.set('reference', '../../..')
    properties = XML.SubElement(def_view, 'properties')
    ...
    for tag_name, default in settings:
        xml_config = XML.SubElement(def_view, tag_name)
        if isinstance(default, bool):
            xml_config.text = str(default).lower()
        else:
            xml_config.text = str(default)
...
def delivery_pipeline(parser, xml_parent, data):
    ...
    """
    _default_view(parser, xml_parent)
    ...
    " default not configurable options"
    owner = XML.SubElement(delivery, 'owner')
    owner.set('class', 'com.cloudbees.hudson.plugins.folder.Folder')
    owner.set('reference', '../../..')
    properties = XML.SubElement(delivery, 'properties')
    properties.set('class', 'hudson.model.View$PropertyList')
